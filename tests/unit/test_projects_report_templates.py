...
* **The catalogue route.** ``GET /projects/reports/templates`` answers all 13
  templates, and exactly ``team_pulse``, ``weekly_delivery``,
  ``project_status`` and ``data_hygiene`` are live (``project_status``
  since WS-27bn R3a, ``data_hygiene`` since R3c, ``team_pulse`` since
  R3d). The call goes through a
