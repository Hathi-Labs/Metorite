...
/**
 * Organisation — the one admin destination for the company (D49).
 *
 ...
 *     where the interesting work happens; this is the list that gets you there).
 ...
 * is what that means, so the seat and branding surfaces join the strip this page
 * already had rather than a second strip nested inside a shell.
 *
 * ## The Requests tab is the other direction
 *
 * Until it existed this screen only showed people an admin had already thought
 * of; somebody arriving at the front door produced a log line nobody read. The
 * badge is the whole point — the owner is supposed to learn that a colleague is
 * locked out without being told.
 *
 * ## The roster knows who is reading it (N7)
 *
 * It used to not, so it drew **Suspend** on the viewer's own row like any
 * other — the one click that takes an owner out of the surface they would need
 * to undo it. `rowActions` (./lib/selfGuard) decides what a row may offer; the
 * refusal itself is the gateway's, and this is only a courtesy
 * (`lib/access.ts`).
 */
    ...
    <div className="flex h-full flex-col">
      ...
      <Tabs
        ...
        tabs={[
          ...
          },
          {
            ...
            icon: "UserCheck",
            note: "Who holds a paid seat, and who is unassigned",
          },
          {
            id: "branding",
            label: "Branding",
            icon: "Building2",
            note: "Your logo, as every member sees it",
          },
          {
            id: "requests",
            label: "Requests",
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 sm:px-6">
        {tab === "branding" ? (
          ...
          // a write via `load` — an assign that changed nothing visible would be
          // indistinguishable from one that failed silently.
          <SeatsTab members={members} loading={loading} onChanged={load} />
        ) : loading ? (
...
/**
 * Invite a member — and, by default, seat them (D49 · `launch_surface.md` §6.3).
 *
 * "Assign a seat now" is ticked by default because seating a colleague as you
 * invite them is the normal onboarding order, and the alternative default makes
 * every invite a two-step job an admin has to remember the second half of.
 *
 * ⚠️ **The invite is never rolled back by a failed assign.** Two systems, two
 * transactions, no way to span them — and a member who exists without a seat is
 * precisely the *Unassigned* state the Seats tab is built to fix. Unwinding a
 * membership because billing was briefly unreachable would throw away the
 * durable half of the work to tidy the transient half. `lib/inviteSeat.ts` owns
 * the four outcomes and the sentence for each; this component only drives the
 * two requests.
 */
function InviteDialog({
  ...
  const [role, setRole] = useState("member");
  const [seatToo, setSeatToo] = useState(true);
  const [busy, setBusy] = useState(false);
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (seatToo) {
        try {
          ...
          const r = await fetch("/api/billing/seats/assign", {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify(assignBody(target, "core")),
          });
          seat = interpretSeatAction(r.status, await r.json().catch(() => null));
        } catch {
          seat = {
            kind: "error" as const,
            message: "Could not reach seat management.",
          };
        }
      }
      ...
      if (memberExists(result)) await onDone();
      if (!shouldStayOpen(result)) onClose();
    } catch (e) {
      setOutcome(
        fromInviteFailure(e instanceof Error ? e.message : "Invite failed."),
      );
    } finally {
      setBusy(false);
    }
      ...
      <div className="relative w-full max-w-md rounded-xl border border-border bg-card p-5 shadow-2xl">
        ...
        <label className="mb-4 flex items-start gap-2 text-xs text-muted-foreground">
          <input
            type="checkbox"
            checked={seatToo}
            onChange={(e) => setSeatToo(e.target.checked)}
          ...
          <span>
            ...
            <br />
            Uses one paid seat. Without it they join as{" "}
            <em>unassigned</em> and can be seated later from Seat assignments.
          </span>
        </label>

        {/* One message slot, three meanings — the copy names which
            (`lib/inviteSeat.ts`). A member that EXISTS is never reported in a
            way that reads as a failed invite, or an admin re-sends it. */}
        {outcome && (
          <p
            className={`mb-3 rounded-lg border px-3 py-2 text-xs ${
              memberExists(outcome)
                ? "border-warning/30 bg-warning/10 text-warning"
                : "border-destructive/30 bg-destructive/10 text-destructive"
            }`}
          >
            {describeInvite(outcome, displayName.trim() || email.trim())}
          </p>
        )}
