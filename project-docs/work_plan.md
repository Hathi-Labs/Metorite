...
| **WS-39** | **Projects · Tasks · Calendar — the re-plumbing** *(minted 2026-08-24)* | ✅ **S1 BUILT** (PR #91) · ✅ **S2 BUILT** · 🟢 S3a next · 🔴 S3b/S3c owner-gated | `project_management_app.md` **§12** + `task_manager_app.md` **§13** + `calendar_focus_os.md` **§10** · **D52 · D53 · D54** | Owner directive 2026-08-24, three decisions in one breath: kill ClickUp, make Tasks the personal lens over Projects, lift Calendar into its own app. **Five slices, sequenced below — the order is load-bearing, not a preference.** 🔴 Slices **S3b** and **S3c** (the `gtd_*` backfill and drop against a live database) are **OWNER-GATE**, registered in §6 by D53.5. |
