...
## 3. Decisions recorded (D1–D14: 2026-07-31→08-04 · D15/D16: 2026-08-08 · D17–D21: 2026-08-09 · D22–D30: 2026-08-10 · D31: 2026-08-11 · D32–D35: 2026-08-12 · D36–D38: 2026-08-13 · D39: 2026-08-14 · D40: 2026-08-17 · D41/D42: 2026-08-18 · D43: 2026-08-19 · D44/D45: 2026-08-19, **in flight on `governance-d45-owner-grants`** — the numbers are TAKEN, do not reuse them · D46/D47: 2026-08-19 · D48: 2026-08-22 · **D49: 2026-08-24** · **D50: 2026-08-24**)
...
- **D48 — H6 identity cutover re-keys RBAC onto `user_identity`, resolved in TWO phases,
  with status mirrored FORWARD.** *(owner-ratified, 2026-08-22.)*
...
>
> **WS-29 / MT-1f — turning per-tenant subdomains ON.** *(Registered 2026-08-24 with
> the owner's B1–B7 rulings; `saas_multitenancy.md` §11 MT-1f.)* Building the host
> parser, the proxy branch, the reserved-label vocabulary and the fences is
> **AGENT-SAFE** and ships dark. **Five runtime acts are OWNER-GATE and an agent must
> refuse each by name:** (a) publishing the wildcard DNS record `*.metorite.com`;
> (b) obtaining the **DNS-01 wildcard TLS certificate** and editing
> `deploy/hostinger/caddy/Caddyfile` to serve it — on-demand TLS is **rejected**
> (it discloses every customer slug to the CT logs and makes the handshake an
> existence oracle); (c) setting **`AUTH_URL=https://app.metorite.com`** in the
> workbench env — a **HARD prerequisite of the flip**, because `auth.ts:219` sets
> `trustHost: true` and no `AUTH_URL`/`NEXTAUTH_URL` exists, so a wildcard-host
> request would derive an OAuth `redirect_uri` on an unregistered hostname;
> (d) setting `SUBDOMAIN_WORKSPACE_ENABLED=true`; and — added 2026-08-24 in slice
> 1's repair round, where the verifier found it missing — (e) setting
> **`WORKSPACE_BASE_DOMAIN`** whenever the apex is not `metorite.com`, which is
> `proxy.ts`'s default. Unset on a different apex the feature **fails safe and
> silently**: nothing matches, no request is touched, and the flip appears to do
> nothing — the right failure, and precisely the one that costs an operator an
> afternoon, so it rides the flip checklist rather than a code comment. Any of
> (a)–(e) attempted by an agent is the refusal; the Caddy patch text belongs in a PR
> body, never in a commit under `deploy/`.
