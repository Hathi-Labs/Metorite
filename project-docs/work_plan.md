...
### D76 — One home for every work fact: My Tasks reads the Projects fields (2026-09-23)
...
**⚠️ This AMENDS D53.8, by owner directive, for two fields.** D53.8 kept the
matrix's `important` on the overlay, apart from `pm_tasks.importance`. It also
kept the overlay's own `time_estimate_mins` beside `pm_tasks.estimate_mins`.
The owner's words above overrule both. Every other part of D53.7 and D53.8
stands.
...
1. `importance` vs the matrix cell. A task Projects called Urgent read
   `Low Priority · Eliminate?` in My Tasks.
...
| Important flags | carried into `importance` | Migration 215 raises an unset, Low or Normal Priority to High where the chosen flag is true. It never lowers a Priority. When no assignee holds a flag, a non-assignee's flag counts, the same rule as for estimates |
...
**Retired, not dropped (R6).** The overlay's `important` and
`time_estimate_mins` stay as columns. Nothing reads them. `validate_overlay`
refuses a write with a 422, and `_upsert_personal` refuses one with a
ValueError. A later contract drops the columns.

**Fences:** `tests/unit/test_projects_personal_s6f.py`,
`tests/live/live_ws39_s6f.py` (12 checks), `sharedFields.test.ts`,
