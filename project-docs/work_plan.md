...
| **WS-39** | **Projects · Tasks · Calendar — the re-plumbing** *(minted 2026-08-24)* | 🟢 **S1 dispatchable** · S2–S5 sequenced | `project_management_app.md` **§12** + `task_manager_app.md` **§13** + `calendar_focus_os.md` **§10** · **D52 · D53 · D54** | Owner directive 2026-08-24, three decisions in one breath: kill ClickUp, make Tasks the personal lens over Projects, lift Calendar into its own app. **Five slices, sequenced below — the order is load-bearing, not a preference.** 🔴 Slices **S3b** and **S3c** (the `gtd_*` backfill and drop against a live database) are **OWNER-GATE**, registered in §6 by D53.5. |
...
|---|---|---|---|
| **S1** | **ClickUp excision** — delete the ingestion source, `skill-clickup-sync`, the `ClickUpProvider` arm, the poll scheduler, both importers, the frontend `ImportClickUp.tsx`, the catalog/OAuth/env entries, and the `scripts/clickup_sync.py` one-off. Columns STAY (D52.3). | 🟢 AGENT-SAFE | Nothing else depends on it landing first, and it is the only slice that is pure deletion — so it is the one that can be verified by "does anything still import this". Doing it first stops the other four having to preserve a dead arm. |
| **S2** | **The Calendar app shell** — `/tasks/calendar` → `/calendar`, a ninth `live` pane in Personal Center, `nav.test.ts`'s count fence moved 8→9, `launch_surface.md` §2 updated. Still reading `gtd_items`. | 🟢 AGENT-SAFE | Deliberately BEFORE the store move. It is a route + registry change with no data semantics, so it lands and is reviewable on its own; folding it into S3 would put a nav change and a store change in one diff. |
| **S3a** | **Tasks becomes the lens** — the `/tasks` UI re-points from `/api/tasks/*` onto `/projects/my/*`; `gtd_*` writes stop; Calendar's task reads move `gtd_items` → `pm_tasks` in the same PR (D54.5). | 🟢 AGENT-SAFE | The big one. Expand half of R6: new readers, old tables intact, nothing dropped. |
...
- **D54 — Calendar is its own app under Personal Center.** *(owner call, 2026-08-24;
  ...
  - **D54.1 — the surface moves.** `/tasks/calendar` becomes **`/calendar`**, a
    top-level pane in the **Personal Center** section of `src/lib/nav.ts`, gated
    `feature:calendar`.
