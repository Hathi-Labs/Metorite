...
- **D64 — The Operator Console gets real staff identity: one directory, three
  ...
  **one shared passphrase** (`OPERATOR_CONSOLE_STAFF_SECRET`). The cookie holds
  ...
  1. **Supabase Auth, with the Microsoft provider, authenticates staff.** This
  ...
  2. **Three checks admit an operator, and all three must pass** — the directory
     tenant id, the email domain, and a row in a new `operator` registry. The
...
| **B5** | **Configure the Supabase Auth Microsoft provider for STAFF**, and set `OPERATOR_ENTRA_TENANT_ID`, `OPERATOR_STAFF_DOMAINS` and `OPERATOR_BOOTSTRAP_EMAIL` (**D64**, CP-12a) | 🔴 **The Operator Console stays on one shared passphrase.** It has been deployed and reachable since 2026-08-22 with no per-person identity, no roles, and an audit log that records the literal `operator` for every write. CP-12a…g are all AGENT-SAFE to build without this — but none of them can be turned on |
...
| **C4** | **Name the first operators and their roles** (**D64.3**, CP-12d) | Nobody can be added until the owner says who is a `viewer`, who is an `editor` and who is an `admin`. ⚠️ Naming a **second** `admin` is also the trigger that pulls four-eyes approval (**DEF-1**) out of deferral — the two decisions arrive together and must not be separated |
...
> **D64**.)* Building all seven slices and R8-testing them against scratch databases is
> **AGENT-SAFE and unblocked.** Seven acts are **OWNER-GATE and must be refused by name**:
> **(a)** configuring the Supabase Auth Microsoft provider for staff, and holding its
> client secret — the external-account class of §6.0 B;
> **(b)** setting `OPERATOR_ENTRA_TENANT_ID`, `OPERATOR_STAFF_DOMAINS` or
...
> builds, and that is its entire purpose. An agent that believes it needs this has
> misread the ticket;
