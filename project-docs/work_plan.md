...
| **S3a-client** *(slice 1)* | ✅ **BUILT 2026-08-25.** **The spine re-points, behind a flag.** `NEXT_PUBLIC_TASKS_LENS` (default **OFF**) + `app/tasks/lib/lens.ts` — a `GtdItem` adapter over `/projects/my/*` — and eight functions in `tasks/lib/api.ts` branching to it: `fetchItems`, `apiCapture`, `apiPatchItem`, `apiArchiveItem`, `apiDeleteItem`, `apiRestoreItem`, `apiPurgeItem`, `apiDelegateItem`. Server addendum: `GET /projects/my/tasks/{id}`, `include_archived`, `CaptureIn.notes`, and the three facts the projection was dropping. 🟢 **Slice 2 next** — the AI, subtask, plan, bulk and workspace tail (H-33). | 🟢 AGENT-SAFE | Expand half of R6: new readers, old tables intact, nothing dropped. **The flag is not optional and cannot be flipped yet** — `gtd_items` still holds every existing task and S3b has not run, so an early flip does not degrade the app, it EMPTIES it, on a 200. |
