...
| **M2.9d** | 🆕 **The `decide` task — fast typed decisions from TypeSafe's Jev (CP-13, D75).** Operator Console first, then the app layer. Detail: `customer_console.md` **§6A.14** | Nothing serves a decision today. Email triage, the commitment gate and the meeting copilot ask a chat model and parse its JSON | 🟢 CP-13a to CP-13d AGENT-SAFE · 🔴 the account, the key, and tenant content to a new sub-processor (§6.1 WS-31 (i)) |
