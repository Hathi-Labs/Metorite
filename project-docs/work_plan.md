...
| WS-34 | **Launch surface — what a customer sees on day one** *(minted 2026-08-24, D49)* | 🟢 ready | **`specs/launch_surface.md`** | The owner directive of 2026-08-24, specced: flat ₹500/user/month + AI credits replacing the Center-package ladder (supersedes D23/D24 as the pricing shape of record) · the "Centers" nav section deleted while **every line of Center code, every `center.*` feature and every `group:<slug>` grant stays** · an eight-pane launch allowlist with everything else `preview` (routes intact, nav entry gone, `NEXT_PUBLIC_SHOW_PREVIEW_APPS` default OFF) · seats a customer admin drives from Organisation (invite → assign → release → **Unassigned** → reassign) with operator parity · and the three nav-resolution defects behind the reported "sometimes all the apps appear" flash (§8: `visibleSections(null)` returns EVERYTHING, `fetchAccess` maps a 502 to NO_ACCESS, and the home grid filters nothing at all). LS-1…LS-10 AGENT-SAFE; **LS-11 OWNER-GATE** (seats already held on a retired plan). |
