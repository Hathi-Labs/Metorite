...
> **AGENT-SAFE** and ships dark. **Four runtime acts are OWNER-GATE and an agent must
...
> request would derive an OAuth `redirect_uri` on an unregistered hostname; and
> (d) setting `SUBDOMAIN_WORKSPACE_ENABLED=true`. Any of (a)–(d) attempted by an
> agent is the refusal; the Caddy patch text belongs in a PR body, never in a commit
> under `deploy/`.
