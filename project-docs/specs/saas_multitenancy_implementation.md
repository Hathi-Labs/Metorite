...
| 6b | ~~`ON CONFLICT (email)` after migration 162~~ → **FIXED 2026-08-19 (MT-1j slice 6)** | 162 dropped `app_user_email_key`, leaving only `app_user_email_lower_key ON (lower(email))`; both upserts named the bare column. Reproduced RED on a real ladder as required — **42P10**, and at **plan** time, so the fresh-insert path failed too and `app_user` stayed empty. Both now target `(lower(email))`. The trap that survives: a **new** upsert written against the byte-exact index, which is why the fix shipped with a whitespace-proof ratchet over `packages/` + `apps/` in `tests/unit/test_app_user_upserts.py` (one allow-listed site, the *Console* plane, whose `CITEXT NOT NULL UNIQUE` reason is itself checked) |
