...
| **T-6** | Close H-10's blind window | A PR that is conflicted with `main` still runs the migration-collision fence; proven by opening a deliberately conflicted PR carrying a duplicate migration number and watching it go red. ⚠️ **HALF landed on PR #46** (open, `ws-27-pr-check-blind-window`): `pr-check.yml` now also triggers on `push`, so a conflicted PR's branch is checked even though GitHub computes no `refs/pull/N/merge` for it. The **cross-branch** half is untouched and is what remains here — `test_migration_prefixes.py` globs the WORKING TREE, so two branches each adding `172_*.sql` are individually green and the duplicate exists only in the merge result, which nothing materialises. That needs `merge_group:` or a checkout-head-then-merge-base job; the acceptance above is unchanged and is the proof | 🟢 AGENT-SAFE |
