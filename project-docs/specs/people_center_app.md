...
| | `app_user` (+ `user_role`, `org_group_member`) | `people` (+ `people_resumes`) |
|---|---|---|
...
⚠️ **`people.name` is no longer UNIQUE and `email` is** (partial, `WHERE email IS NOT
...
**Where it lives:** a column on `people`, as a data URI — the shape
...
| **`people_skills`** | The **structured** skill: level, years, last used, evidence. See below | H | S | ✅ 175 |
| `resume_summary`, `years_experience`, `domain` | Résumé depth, for seniority weighting | H | S | ✅ 49 |
| `people_resumes` | The CV itself + its parse | H | S | ✅ 74 |
| **`people_credentials`** | Education, certifications, and prior roles, extracted from the CV or typed — "is this person actually qualified to sign this off" | H | S | ✅ 175 |
...
`people_skills` rewrites `people.skills` and `skills_source` **in the same
...
| **`people_absences`** | Away from when to when, and roughly why. See §5.8 | H | S | 🔲 P-5 |
...
⚠️ **`people.organization_id` is on the NUMBERED ladder now.** It was
...
`people.manager_id` is a self-FK, so the chart is the same recursive render the project
...
1. **Stated skills** — exact/fuzzy match on `skills[]`, now weighted by level and recency
   (§3.3). Deterministic.
2. **Résumé evidence** — a match in `people_resumes.extracted`, quoting the line.
...
> module's only SELECT reads `people`". That one statement is the
...
> write fill only `people.skills` — the panel asserted "nobody claims
...
One tool, `people_directory`, replacing nothing: `people(query)` already exists for the
...
### 7.0 The tables lost their `gtd_` prefix on 2026-09-21

Owner directive: *"I really don't want GTD anymore... update the naming
convention for all of the table names accordingly."* The People family moved
first, and this spec speaks only the new names from here on.
...
**The rename lives inside the migration that CREATES each table** (49, 74, 174,
176), as a guarded prologue. One file then answers all three states. A fresh
install makes the new name directly. An existing database is renamed with its
rows. A replay finds the work done and does nothing. A rename migration at the
END of the ladder was built first, and it broke eight earlier files on replay,
because each one addresses a name that is gone.

**Index and constraint names keep the old spelling.** A rename does not touch
them, migration 148 looks some of them up by name, and no query names an index.
`uq_gtd_people_org_email_lower` and `gtd_people_status_check` are correct as
written.

⚠️ **`people` is a prefix of its own family.** `"FROM gtd_people" in sql` was an
exact test and `"FROM people" in sql` is not. It also matches
`people_absences`. `tests/unit/_sql_match.py` is the one helper for this, and
`tests/unit/test_people_rename_upgrade.py` is the fence for the rename itself.

**No new people STORE.** The People Center reads `people`, `people_resumes`,
...
Done when: `people` no longer uniquely constrains `name`; a partial unique index exists
...
- **One field-class authority in code** (`routes/people/fields.py`): every writable column
  ...
  column of `people` — so a column added later fails the test rather than defaulting
...
- **The direction has a structural fence**, not a paragraph: a test sweeps the whole tasks
  package for a write to `people.working_hours` and fails on one. Prose about a
...
Done when: `people_skills` carries level, years, last-used and evidence; **every write
path rewrites `people.skills`/`skills_source` in the same transaction** and a test
...
fence around the HR projection, and any new read path over `people` must leave them
...
| **D-PC-6** | Structured skills go in a child table; `people.skills[]` becomes a **maintained projection**, rewritten in the same transaction | Four live consumers read the array (GIN index, `_match_capability`, clarify, directory filters) and R6 forbids breaking running code. Fence: a route test comparing array to table after every write |
