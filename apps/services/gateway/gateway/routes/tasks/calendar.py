...
def _candidate_brief(m: Any, now: datetime) -> dict[str, Any]:
    ...
    return {
        ...
        # D78: Important and Leveraged are shared facts on the task. The
        # planner reads them from `pm_tasks`, the same rule the client's
        # `priorityInputs` applies, so the list and "Plan my day" agree.
        "important": bool(
            important_from_importance(getattr(m, "importance", None))
        ),
        "leveraged": bool(getattr(m, "leveraged", None)),
