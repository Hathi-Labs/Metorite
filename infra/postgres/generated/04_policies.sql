...
ALTER TABLE pm_report_recipients ENABLE ROW LEVEL SECURITY;
ALTER TABLE pm_report_recipients FORCE  ROW LEVEL SECURITY;
DROP POLICY IF EXISTS pm_report_recipients_tenant_isolation ON pm_report_recipients;
CREATE POLICY pm_report_recipients_tenant_isolation ON pm_report_recipients
    USING      (organization_id = current_setting('app.tenant_id', true)::uuid)
    WITH CHECK (organization_id = current_setting('app.tenant_id', true)::uuid);
